...
import catalogI18n from "@/data/catalog-i18n.json"
 ...
 */
/** Catalogue text in one language, falling back to the Azerbaijani source. */
const say = (
...
/**
 * One catalogue item in one language.
 *
 * The translations are generated once and committed (src/data/catalog-i18n.json)
 * rather than fetched: 1168 tests cannot be translated at render time, and the
 * catalogue does not change between builds. Anything untranslated falls back to
 * the Azerbaijani, which is the name the laboratory actually prints.
 */
export const localizeItem = (item: CatalogItem, locale: string): CatalogItem =>
  locale === "az"
    ? item
    : {
        ...item,
        name: say(catalogI18n.names, item.name, locale),
        description: say(catalogI18n.descriptions, item.description, locale),
        prep: say(catalogI18n.preps, item.prep, locale),
...
export const pickerGroups = (locale: string = "az") =>
  GROUP_ORDER.map((group) => {
    const categories = getGroupCategories(group.slug).map((category) => ({
      slug: category.slug,
      name: say(catalogI18n.categories, category.name, locale),
    ...
    categories.unshift({
      ...
      name: say(catalogI18n.labels, entry?.label ?? "Populyar", locale),
    ...
    return {
      ...group,
      name: say(catalogI18n.groups, group.name, locale),
      blurb: say(catalogI18n.groups, group.blurb, locale),
